<%--
  Class Name : EgovSrProcessDetail.jsp
  Description : SR처리 상세내역
...
<title><spring:message code='sr.processDetail'/></title>
...
async function loadData(payload = {}, dataType = "json") {
  return new Promise((resolve, reject) => {
  	...
  	$("#pageIndex").val(Number(payload.pageNo) + 1);
 		$.ajax({
	    	url : "<c:url value='/select/sts/prcdt/selectSrProcessDetail.do'/>",
	   	    type : 'POST',
	   	    //dataType : "json",
...
function createGrid() {
   let dsConfig = {
	...
	let gridConfig = {
		...
		{ field: 'requestDate', caption: "<spring:message code='sr.requestDate'/>", editable: false, type: 'text', width:6, unit:'%'},
		{ field: 'srNo', caption: "<spring:message code='sr.srNumber'/>", editable: false, type: 'text', valign: 'center' , width:6, unit:'%'},
		...
		{ field: 'testCompleteDate', caption: "<spring:message code='sr.customersConfirmationCompletionDate'/>", editable: false, type: 'text', width:6, unit:'%'},
		{ field: 'transferDate', caption: "<spring:message code='sr.transferDate'/>", editable: false, type: 'text', width:6, unit:'%'},
		{ field: 'answerSe', caption: "<spring:message code='sr.gubun'/>", editable: false, type: 'text', width:6 , unit:'%'
			,getValue : (value, field, rowItem) => {
				var text = "";
				if(value == "10"){
					text = "<spring:message code='sr.answers' />";
				}else if(value == "20"){
					text = "<spring:message code='sr.addRequest' />";
				}				
				return text;
			}},
		{ field: 'expectTime', caption: "<spring:message code='sr.predictionLabourHours'/>", editable: false, type: 'text', width:6, unit:'%'},
		...
		{ field: 'completeDate', caption: "<spring:message code='sr.expectedCompletionDate'/>", editable: false, type: 'text', width:6, unit:'%'},
...
function fn_egov_pageview(pageNo){
    ...
    document.listForm.action = "<c:url value='/sts/prcdt/selectSrProcessDetail.do'/>";
 ...
 */
 function fnExcelReport(){
    /*  document.listForm.action = "<c:url value='/sts/prcdt/EgovProcessDetailExcelReport.do'/>";
     document.listForm.submit(); */
	 $.ajax({
	    	url : "<c:url value='/sts/prcdt/EgovProcessDetailExcelReport.do'/>",
	   	    type : 'POST',
	   	    data : $("#listForm").serialize(),
		...
		if(fdate.value > tdate.value){
			...
			if(tReqDate != tdate.value){
				...
				if(tReqDate == ""){
					...
					document.getElementById('searchDateTView').value = tReqDate.substring(0,4) + '-' + tReqDate.substring(4,6) + '-' + tReqDate.substring(6,8);	
				}
			}
...
<div class="sr-contents-wrap">
        ...
        <div class="sr-contents-area">
	        <div class="sr-contents">
	            ...
	            <form name="listForm" id="listForm" action="<c:url value='/sts/prcdt/selectSrProcessDetail.do'/>" method="post">
	                    ...
	                    <tbody>
	                     ...
	                     <tr>
	                     	...
	                     	<th>
	                            ...
	                            <th>
		                            <sbux-label id="table_label4" name="table_label4" uitype="normal" text="<spring:message code='sr.client'/>" class=""></sbux-label>
		                         </th>
		                         <td>
	                     ...
	                     <tr>
	                     	 <th>
		                                <sbux-label id="table_label5" name="table_label5" uitype="normal" text="<spring:message code='sr.status'/>"></sbux-label>
		                            </th>
		                            <td colspan="3">
		                                <c:forEach var="statusCode_result" items="${statusCode_result}" varStatus="status">
											<c:set var="vChecked">
...
<%-- <div id="wrapper">
    ...
    <!-- //header 끝 --> 
        <!-- 현재위치 네비게이션 시작 -->
        <div id="contents">
	        <form name="listForm" action="<c:url value='/sts/prcdt/selectSrProcessDetail.do'/>" method="post">
	        <input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }" />
            ...
            <div id="searchtb">
				<table width="980" border="0" cellpadding="0" cellspacing="0">
            		...
            		<tr>
                   		<td class="tdblue"><spring:message code='sr.requestDate'/></td>
                   		<td class="tdleft" width="270px;">
                  			<input name="searchDateF" id="searchDateF" type="hidden" value="<c:out value='${searchVO.searchDateF}'/>" />
                        	<input name="searchDateFView" id="searchDateFView" title="게시시작일" type="text" size="10" value="<c:out value='${searchVO.searchDateFView}'/>"  readonly="readonly"
                            	...
                            	fnReqDateChk(document.listForm.searchDateF, document.listForm.searchDateT);" >
                        	<img src="<c:url value='/images/calendar.gif' />"
                            	onClick="javascript:fn_egov_NormalCalendar(document.listForm, document.listForm.searchDateF, document.listForm.searchDateFView,'','<c:url value='/sym/ccm/EgovselectNormalCalendar.do'/>');
                            	fnReqDateChk(document.listForm.searchDateF, document.listForm.searchDateT);" width="15" height="15" alt="calendar">
                        	~
                        	<input name="searchDateT" id="searchDateT" type="hidden" value="<c:out value='${searchVO.searchDateT}'/>" />
                        	<input name="searchDateTView" id="searchDateTView" title="게시종료일" type="text" size="10" value="<c:out value='${searchVO.searchDateTView}'/>"  readonly="readonly"
		                ...
		                <td class="tdblue"><spring:message code='sr.completionDate'/></td>
                   		<td class="tdleft" width="270px;">
                  			<input name="searchCompleteDateF" id="searchCompleteDateF" type="hidden" value="<c:out value='${searchVO.searchCompleteDateF}'/>" />
                        	<input name="searchCompleteDateFView" id="searchCompleteDateFView" title="게시시작일" type="text" size="10" value="<c:out value='${searchVO.searchCompleteDateFView}'/>"  readonly="readonly"
                            	onClick="javascript:fn_egov_NormalCalendar(document.listForm, document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateFView,'','<c:url value='/sym/ccm/EgovselectNormalCalendar.do'/>');
                            	fnComDateChk(document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateT);" >
                        	<img src="<c:url value='/images/calendar.gif' />"
                            	onClick="javascript:fn_egov_NormalCalendar(document.listForm, document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateFView,'','<c:url value='/sym/ccm/EgovselectNormalCalendar.do'/>');
                            	fnComDateChk(document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateT);" width="15" height="15" alt="calendar">
                        	~
                        	<input name="searchCompleteDateT" id="searchCompleteDateT" type="hidden" value="<c:out value='${searchVO.searchCompleteDateT}'/>" />
                        	<input name="searchCompleteDateTView" id="searchCompleteDateTView" title="게시종료일" type="text" size="10" value="<c:out value='${searchVO.searchCompleteDateTView}'/>"  readonly="readonly"
	                            onClick="javascript:fn_egov_NormalCalendar(document.listForm, document.listForm.searchCompleteDateT, document.listForm.searchCompleteDateTView,'','<c:url value='/sym/ccm/EgovselectNormalCalendar.do'/>');
	                            fnComDateChk(document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateT);"  >
                        	<img src="<c:url value='/images/calendar.gif' />"
                            	onClick="javascript:fn_egov_NormalCalendar(document.listForm, document.listForm.searchCompleteDateT, document.listForm.searchCompleteDateTView,'','<c:url value='/sym/ccm/EgovselectNormalCalendar.do'/>');
                            	fnComDateChk(document.listForm.searchCompleteDateF, document.listForm.searchCompleteDateT);" width="15" height="15" alt="calendar">
                        	<br/><form:errors path="searchCompleteDateF" />
                        	<br/><form:errors path="searchCompleteDateT" />
                       	</td>
			        ...
			        <c:if test="${authorCode == 'ROLE_ADMIN' || authorCode == 'ROLE_CHARGER' || authorCode == 'ROLE_MNGR'}">
                   	<tr>
						<td class="tdblue"><spring:message code='sr.module'/></td>
                   		<td class="tdleft" width="270px;">
...
<!-- 	                        <div><form:errors path="pstinstCode" cssClass="error" /></div> -->
			        ...
			        <tr>
                   		<td class="tdblue"><spring:message code='sr.status'/></td>
                   		<td class="tdleft" colspan="3">
                   			<c:forEach var="statusCode_result" items="${statusCode_result}" varStatus="status">
                           		<c:set var="vChecked">
                            		<c:forEach var="vList" items="${searchVO.searchStatus}" varStatus="vStatus2">
			...
			<div class="list">
        	<ul>
            	<li><img src="<c:url value='/' />images/sr/bullet_arrow.gif" style="vertical-align:middle"/> <spring:message code='sr.msg.totalCnt'/> <span style="color:#F00"><c:out value='${paginationInfo.totalRecordCount}'/><spring:message code='sr.msg.cnt'/></span></li>
            </ul>
		    </div>
		    <div class="list2">
		    <ul>
		       	<li><img src="<c:url value='/' />images/sr/icon_home.gif"/> &nbsp;HOME > 통계 > SR처리 상세내역</li>
		    </ul>
		    </div>
       		...
       		<div class="list3">
                ...
                <thead>
                	...
                	<tr class="tdgrey">
                     	...
                     	<th scope="col" nowrap="nowrap">SR번호</th>
                     	<th scope="col" nowrap="nowrap">SR제목</th>
                     	<th scope="col" nowrap="nowrap">요청자성명</th>
                     	<th scope="col" nowrap="nowrap">고객확인완료일</th>
                     	<th scope="col" nowrap="nowrap">운영이관일</th>
                     	<th scope="col" nowrap="nowrap">예상공수(H)</th>
                     	<th scope="col" nowrap="nowrap">실적공수(H)</th>
                     	<th scope="col" nowrap="nowrap">모듈<br>(분야)</th>
                     	<th scope="col" nowrap="nowrap">담당자<br>(처리자)</th>
                     	<th scope="col" nowrap="nowrap">해결일자</th>
                 	</tr>
                 	<tr>
                 ...
                 <tbody>
                 	...
                 	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
                   	<tr>
	                 	...
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.requestDate}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.srNo}</td>
	                 	<td class="tdwl"  nowrap="nowrap">${resultInfo.subject}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.requesterName}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.testCompleteDate}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.transferDate}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.expectTime}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.realExpectTime}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.moduleName}</td>
	                 	<td class="tdwc"  nowrap="nowrap">${resultInfo.responserName}</td>
