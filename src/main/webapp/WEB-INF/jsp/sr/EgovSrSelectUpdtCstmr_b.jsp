...
function fnDelete(){
    if (confirm("<spring:message code='common.delete.msg'/>")) {
        ...
        varForm.action           = "<c:url value='/sr/EgovSrRemove.do'/>";
    ...
    <div id="container">
        ...
        <!-- //좌측메뉴 끝 -->
            ...
            <div id="contents">
                ...
                <!-- 검색 필드 박스 시작 -->
                <div id="search_field">
                    <div id="search_field_loc"><h2><strong>SR 상세조회(수정)</strong></h2></div>
                </div>
		        <form:form commandName="srVO" action="${pageContext.request.contextPath}/sr/EgovSrSelectUpdtCstmr.do" name="srVO" method="post" enctype="multipart/form-data">
						    ...
						    <td width="30%">
						    	<select name="moduleCode" id="moduleCode" class="select" title="모듈코드">
								    ...
								    <c:forEach var="result" items="${moduleCode_result}" varStatus="status">
										<option value='<c:out value="${result.code}"/>'  <c:if test="${result.code == srVO.moduleCode}">selected</c:if>><c:out value="${result.codeNm}"/></option>
									</c:forEach>
								 </select>
						    </td>    
						  </tr>  
...
<%-- 				                <form:errors path="category" cssClass="error" /> --%>
				                <select name="category" id="category" class="select" title="처리구분">
								    ...
								    <c:forEach var="result" items="${classCode_result}" varStatus="status">
										<option value='<c:out value="${result.code}"/>'  <c:if test="${result.code == srVO.category}">selected</c:if>><c:out value="${result.codeNm}"/></option>
									</c:forEach>
								 </select>
				            </td>
...
<%-- 				                <form:errors path="priority" cssClass="error" /> --%>
				                <input type="radio" name="priority" class="radio2" value="1000" <c:if test="${srVO.priority == '1000'}"> checked="checked"</c:if>>&nbsp;상
					            <input type="radio" name="priority" class="radio2" value="1001" <c:if test="${srVO.priority == '1001'}"> checked="checked"</c:if>>&nbsp;&nbsp;중
					            <input type="radio" name="priority" class="radio2" value="1002" <c:if test="${srVO.priority == '1002'}"> checked="checked"</c:if>>&nbsp;하
						    </td>    
						  </tr>  
...
<%-- 				                <form:errors path="hopeDate" cssClass="error" /> --%>
				                ...
				                <input name="hopeDateView" title="완료희망일" type="text" size="10" value="<c:out value='${srVO.hopeDate}'/>"  readonly="readonly"
						  ...
						  <tr> 
						    ...
						    <td width="80%" colspan="3">
						    	<textarea id="comment" name="comment"  class="textarea" cols="75" rows="20"  style="width:99%;"><c:out value="${srVO.comment}" escapeXml="false" /></textarea> 
                    ...
                    <div class="buttons" style="padding-top:10px;padding-bottom:10px;">
                       ...
                       <table border="0" cellspacing="0" cellpadding="0" align="center">
                        <tr> 
                          ...
                          <td width="10"></td>
                          <td>
                            <a href="#LINK" onclick="fnDelete(); return false;"><spring:message code="button.delete" /></a> 
                          </td>
                          <td width="10"></td>
                          <td>
                            <a href="#LINK" onclick="fnListPage(); return false;"><spring:message code="button.list" /></a>
                          </td>      
                          <td width="10"></td>
                          <td>
